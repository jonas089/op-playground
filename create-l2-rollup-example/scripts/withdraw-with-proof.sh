...
echo "  Withdrawal hash: $WITHDRAWAL_HASH"
echo "  Prover address:  $OUR_ADDR"
echo "  Game proxy:      $FOUND_GAME_PROXY"
echo "  Game index:      $FOUND_GAME_IDX"
echo ""

echo "--------------------------------------------"
echo "Step 1: Resolve dispute game (after ~90s)"
echo "--------------------------------------------"

echo "cast send $FOUND_GAME_PROXY \\"
echo "  'resolveClaim(uint256,uint256)' 0 0 \\"
...
echo "Check status (2 = DEFENDER_WINS):"
echo "cast call $FOUND_GAME_PROXY 'status()(uint8)' --rpc-url $L1_RPC"
echo ""

echo "--------------------------------------------"
echo "Step 2: Wait for registry finality (~60s)"
echo "--------------------------------------------"
echo ""
...
echo "--------------------------------------------"
echo "Step 3: Finalize withdrawal"
echo "--------------------------------------------"

echo "cast send $PORTAL \\"
echo "  'finalizeWithdrawalTransaction((uint256,address,address,uint256,uint256,bytes))' \\"
echo "  '($NONCE,$OUR_ADDR,$OUR_ADDR,$AMOUNT_WEI,100000,0x)' \\"
echo "  --private-key \$PRIVATE_KEY \\"
echo "  --rpc-url $L1_RPC"
echo ""

echo "--------------------------------------------"
echo "Debug: Check proof status (correct ABI)"
echo "--------------------------------------------"

echo "cast call $PORTAL \\"
echo "  'provenWithdrawals(bytes32,address)(address,uint256)' \\"
echo "  $WITHDRAWAL_HASH $OUR_ADDR \\"
echo "  --rpc-url $L1_RPC"
echo ""
...
echo "Summary"
echo "--------------------------------------------"
echo "WithdrawalHash: $WITHDRAWAL_HASH"
echo "Prover:         $OUR_ADDR"
echo "GameProxy:      $FOUND_GAME_PROXY"
echo ""
